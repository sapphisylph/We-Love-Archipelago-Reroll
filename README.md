...
An [Archipelago](https://archipelago.gg/) implementation for We Love Katamari Reroll + Royal Reverie
...
### Goal
Currently, the goal is to complete every main level, then complete Roll Up the Sun. Each level is locked behind its respective fan (including Roll Up the Sun, which is locked behind Dog), so you must receive every fan from the multiworld in order to goal (except Tomio Hoshino because he's normally postgame).
